...
import { FaBook, FaHeadphones, FaRegEye } from "react-icons/fa";
import SidebarUser from "./SidebarUser";
import orderApi from "../api/orderApi";
const ListEbookScreen = () => {
  ...
  const handleGetMyOrders = async (currentPage = 1) => {
    try {
      const response = await orderApi.getMyEbookOrders({
        page: currentPage,
        pageSize,
      });
              ...
              <>
                ...
                <table className="table">
                  ...
                  <tbody>
                    {orders?.map((order) => (
                      <tr key={order._id}>
                        <td className="align-middle">{order.orderItems[0].bookName}</td>
                        ...
                        <td className="align-middle">
                          {transform(order.itemsPrice, optionCurrency)}
                          </td>
                        <td className="align-middle">
                          <LinkContainer to={`/listenaudio/${order.orderItems[0].product}`}>
                            <Button>
                              <FaHeadphones  />
                            </Button>
                          </LinkContainer>
                        ...
                        <td className="align-middle">
                          <LinkContainer to={`/readbook/${order.orderItems[0].product}`}>
                            <Button>
                              <FaBook  />
                            </Button>
                          </LinkContainer>
                ...
                <div className="container-sm">
                  <div className="flex flex-col items-center gap-y-2">
                    <div className=" w-[250px] h-[250px]">
                      <img className src={images.imgNoOrder} alt="empty cart" />
                    </div>
                    <div className="text-[#0b0303] text-[20px] my-3">
                      <p>Danh sách này trống do bạn chưa có Ebook nào. Bạn hãy mua thêm sách định dạng Ebook của shop để trải nghiệm thử nhé!</p>
                    </div>
                    <div className="pb-5">
                      <Link to={"/"}>
                        <button
                          type="button"
                          className="btn btn-danger w-[150px] "
                        >
                          MUA SẮM NGAY
                        </button>
                      </Link>
...
export default ListEbookScreen;
